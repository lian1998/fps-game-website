...
export class MovementController implements CycleInterface, LoopInterface {
    ...
    playerOctree: Octree; // 用于检测玩家碰撞场景的OCTree
    playerCamera: THREE.Camera; // 玩家相机
    playerCollider: Capsule; // 玩家碰撞模型

    playerOnFloor: boolean = true;
    keyStates: Map<InputEventTYPE, boolean> = new Map();

    playerVelocity: THREE.Vector3 = new THREE.Vector3(); // 玩家速度
    playerDirection: THREE.Vector3 = new THREE.Vector3(); // 玩家键盘运动方向

    init(): void {

        this.playerOctree = GameWorld.worldOctree;
        this.playerCamera = GameInstance.GameView.cameras.get(GameInstance.Cameras.playerCamera);
        ...
        iep.addEventListener(_inputEvent.type, (e: CustomEvent) => {
            ...
            switch (e.detail.type) {
                ...
                case InputEventTYPE.MOVE_FORWARD_DOWN:
                    this.keyStates.set(InputEventTYPE.MOVE_FORWARD_DOWN, true);
                    break;

                case InputEventTYPE.MOVE_BACKWARD_DOWN:
                    this.keyStates.set(InputEventTYPE.MOVE_BACKWARD_DOWN, true);
                    break;

                case InputEventTYPE.MOVE_LEFT_DOWN:
                    this.keyStates.set(InputEventTYPE.MOVE_LEFT_DOWN, true);
                    break;

                case InputEventTYPE.MOVE_RIGHT_DOWN:
                    this.keyStates.set(InputEventTYPE.MOVE_RIGHT_DOWN, true);
                    break;

                case InputEventTYPE.MOVE_FORWARD_UP:
                    this.keyStates.set(InputEventTYPE.MOVE_FORWARD_DOWN, false);
                    break;

                case InputEventTYPE.MOVE_BACKWARD_UP:
                    this.keyStates.set(InputEventTYPE.MOVE_BACKWARD_DOWN, false);
                    break;

                case InputEventTYPE.MOVE_LEFT_UP:
                    this.keyStates.set(InputEventTYPE.MOVE_LEFT_DOWN, false);
                    break;

                case InputEventTYPE.MOVE_RIGHT_UP:
                    this.keyStates.set(InputEventTYPE.MOVE_RIGHT_DOWN, false);
                    break;

                case InputEventTYPE.JUMP: // 跳跃
                    this.jump();
                    break;
    ...
    controls(deltaTime: number): void {
        ...
        if (this.playerOnFloor) {
            ...
            if (this.keyStates.get(InputEventTYPE.MOVE_FORWARD_DOWN))
                this.playerDirection.add(this.getForwardVector().normalize());

            if (this.keyStates.get(InputEventTYPE.MOVE_BACKWARD_DOWN))
                this.playerDirection.add(this.getForwardVector().normalize().multiplyScalar(-1));

            if (this.keyStates.get(InputEventTYPE.MOVE_LEFT_DOWN))
                this.playerDirection.add(this.getSideVector().normalize().multiplyScalar(-1));

            if (this.keyStates.get(InputEventTYPE.MOVE_RIGHT_DOWN))
