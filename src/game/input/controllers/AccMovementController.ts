...
const v3Util: THREE.Vector3 = new THREE.Vector3();
const vhorizon = new THREE.Vector3();
...
class AccMovementController implements LoopInterface, CycleInterface {
    ...
    startPoint: THREE.Vector3 = new THREE.Vector3(0, config.capsuleRadius, 0);
    endPoint: THREE.Vector3 = new THREE.Vector3(0, config.capsuleHeight - config.capsuleRadius, 0);
    playerCollider: Capsule = new Capsule(this.startPoint, this.endPoint, config.capsuleRadius); // 玩家的碰撞体积是一个胶囊体 0 ~ 1.8
    velocity: THREE.Vector3 = new THREE.Vector3(); // 存储当前的速度
    moveDirection: THREE.Vector3 = new THREE.Vector3(); // 存储当前的加速度方向(键盘按键)

    init(): void {

        this.worldOctree = GameWorld.worldOctree;
